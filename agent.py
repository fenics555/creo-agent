...
else if(a=='do_role'){var lg=el.getAttribute('data-login');var sel=document.querySelector('.rsel[data-login="'+lg+'"]');J('/admin/users',{token:TK,op:'role',login:lg,role:sel.value}).then(function(r){alert(r.msg||'ок')})}
else if(a=='do_resetpw'){var lg=el.getAttribute('data-login');var nw=prompt('Новый пароль для '+lg+' (мин 4):');if(nw)J('/admin/users',{token:TK,op:'resetpw',login:lg,pw:nw}).then(function(r){alert(r.msg||'ок')})}
...
else if(a=='login')J('/login',{login:lg.value,pw:pw.value}).catch(function(e){alert('сервер недоступен: '+e);throw e}).then(function(r){if(r.ok){TK=r.token;localStorage.setItem('tk',TK);localStorage.setItem('usr',lg.value);login.style.display='none';init()}else alert('неверный логин или пароль')});
