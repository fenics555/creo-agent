...
REGISTRY = [
    ...
    ("Главное", "num_predict", "Длина ответа (макс. токенов)", "int", 2048, "Сколько токенов может быть в ответе. Мало — ответ обрезается («обрезки»).", True),
...
def list_ui():
    ...
    B = {"log_mode": (0, 3, 1), "night_hour": (0, 23, 1), "night_minute": (0, 59, 1),
         ...
         "num_ctx": (4096, 262144, 4096), "num_predict": (512, 32768, 512),
