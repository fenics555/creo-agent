...
def _secret(key, fallback):
    try:
        import json as _j
        d = _j.load(open(r"D:\AI\tools\agent\data\secrets.json", encoding="utf-8"))
...
def login():
    global TOKEN
    l = _secret("qa_login", "qa_tester")
    p = _secret("qa_pw", "QaTester2_2026")
    r = post("/login", {"login": l, "pw": p})
    if r.get("ok"):
        TOKEN = r.get("token")
        return TOKEN
    post("/register", {"login": l, "pw": p})
    r = post("/login", {"login": l, "pw": p})
    TOKEN = r.get("token")
    return TOKEN
