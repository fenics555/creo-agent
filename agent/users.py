...
import json, secrets, hashlib, time, os
...
def _load():
    if not UFILE.exists():
        ...
        tmp = UFILE.with_suffix(".tmp")
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump({"users": []}, f, ensure_ascii=False, indent=2)
        os.replace(tmp, UFILE)
    try:
...
def is_admin(login_or_dict):
    if isinstance(login_or_dict, dict):
        login = login_or_dict.get("login")
    else:
        login = login_or_dict
    u = _find(_load(), login)
