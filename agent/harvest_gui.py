# -*- coding: utf-8 -*-
\"\"\"harvest_gui.py — окно сканера (спека 101, tkinter).
Дизайн: палитра Давыдовки.
\"\"\"
...
class App:
    def __init__(self, root):
        self.root = root
        root.title(\"ХАРВЕСТ — сканер дома (спека 101)\")
    ...
    def _build(self):
        ...
        tk.Label(top, text=\"СКАНЕР ДОМА\", bg=CARD, font=FONT_H).pack(side=\"left\", padx=10)
        self.status = tk.Label(top, text=\"готово\", bg=CARD, font=FONT_S, padx=10)
        self.status.pack(side=\"right\", padx=10)
        main_frame = tk.Frame(self.root, bg=BG)
        main_frame.pack(fill=\"both\", expand=True, padx=16, pady=8)
        set_frame = tk.LabelFrame(main_frame, text=\"НАСТРОЙКИ\", bg=CARD, font=FONT_S, padx=10, pady=10)
        set_frame.pack(side=\"left\", fill=\"y\", padx=(0, 8))
        tk.Label(set_frame, text=\"Расширения:\", bg=CARD, font=FONT_S).pack(anchor=\"w\")
        tk.Entry(set_frame, textvariable=self.var_exts, width=20).pack(fill=\"x\", pady=(0, 10))
        tk.Checkbutton(set_frame, text=\"Маркеры\", variable=self.var_markers, bg=CARD, font=FONT_S).pack(anchor=\"w\")
        tk.Label(set_frame, text=\"Источники:\", bg=CARD, font=FONT_S).pack(anchor=\"w\", pady=(5, 0))
        ...
        tk.Label(set_frame, text=\"Опции:\", bg=CARD, font=FONT_S).pack(anchor=\"w\", pady=(10, 0))
        tk.Checkbutton(set_frame, text=\"Текст\", variable=self.var_text, bg=CARD, font=FONT_S).pack(anchor=\"w\")
        tk.Checkbutton(set_frame, text=\"Bench\", variable=self.var_bench, bg=CARD, font=FONT_S).pack(anchor=\"w\")
        tk.Checkbutton(set_frame, text=\"Z:\", variable=self.var_z, bg=CARD, font=FONT_S).pack(anchor=\"w\")
        tk.Button(set_frame, text=\"ЗАПУСК\", command=self.on_scan, bg=ACCENT, fg=\"white\", font=FONT_S, height=2).pack(fill=\"x\", pady=(20, 0))
        tk.Button(set_frame, text=\"СТОП\", command=self.on_stop, bg=ERR_BG, fg=TXT, font=FONT_S).pack(fill=\"x\", pady=(5, 0))
    ...
    def on_scan(self):
        ...
        new_sett = {
            ...
            \"markers\": self.var_markers.get(),
            \"chunks_sources\": srcs,
            \"batch\": self.var_batch.get(),
            \"text\": self.var_text.get(),
            \"bench\": self.var_bench.get(),
            \"z_allowed\": self.var_z.get(),
            \"roots_checked\": roots
        }
