...
g.tools.forEach(function(tl){h+'<div class="tool" data-act="chip" data-val="'+att(tl.name)+'" data-desc="'+att(tl.desc||'')+'"><b class="tool-title">'+((tl.icon||'')?tl.icon+' ':'')+esc(tl.name)+(tl.approval?' 🔒':'')+'<span class="tip" data-act="tip" data-val="'+att(tl.name)+''>?</span></b><small>'+esc(tl.desc||'')+'</small></div><div class="tip-desc" style="display:none" data-tool-id="'+att(tl.name)+''>'+esc(tl.desc||'')+'</div>'});h+'</div></div>'});
panel.innerHTML=h;
...
else if(a=='details'){var k=el.getAttribute('data-val');qinp.value=(k=='dbfull')?'db_state verbose=1':'guide topic='+k;send()}else if(a=='details-toggle'){var cont=el.nextElementSibling;cont.style.display=(cont.style.display=='none'?'block':'none');} else if(a=='tip'){var val=el.getAttribute('data-val');var descEl=document.querySelector('[data-tool-id="'+val+'"]');var tip=document.getElementById('tooltip');if(descEl&&tip){tip.textContent=descEl.textContent;tip.style.display='block';var rect=el.getBoundingClientRect();tip.style.left=(rect.left+window.scrollX)+'px';tip.style.top=(rect.bottom+window.scrollY+5)+'px';}}
...
function chatRender(ms){var box=document.getElementById('cmsg');ms.forEach(function(m){if(m.id<=CLAST)return;CLAST=m.id;var d=document.createElement('div');d.style.cssText='background:#2C2D30;border-radius:6px;padding:6px 8px';d.innerHTML='<b style="color:#4C8FD6">'+esc(m.name)+'</b> <small style="color:#A6A8AB">'+esc(m.ts)+'</small><br>'+(function(atxt){atxt=atxt.replace(TAGS_RE, "");atxt=atxt.replace(TAGS_RE, "");return atxt})(esc(m.text));box.appendChild(d)});box.scrollTop=box.scrollHeight}
...
document.addEventListener('change',function(e){var r=e.target;var k=r.getAttribute&&r.getAttribute('data-cfg');if(!k)return;
if(r.type!='range'&&r.type!='checkbox'&&r.tagName!='SELECT')return;
var v=(r.type=='checkbox')?(r.checked?1:0):r.value;
fetch('/setcfg',{method:'POST',headers:{'Content-Type':'application/json','X-Token':TK||''},body:JSON.stringify({key:k,value:v})}).then(function(){r.classList.add('flash');setTimeout(function(){r.classList.remove('flash')},1000)});
var lab=r.parentNode&&r.parentNode.querySelector('[data-v]');if(lab&&r.type=='range')lab.textContent=r.value});
document.addEventListener('input',function(e){var r=e.target;if(r&&r.type=='range'&&r.getAttribute('data-cfg')){var lab=r.parentNode.querySelector('[data-v]');if(lab)lab.textContent=r.value}});
